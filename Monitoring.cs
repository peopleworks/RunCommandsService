...
Message:  ${CustomMessage}";
    ...
    public class ExecutionMonitor
    {
        ...
        public void Record(ExecutionEvent ev)
        {
            ...
            _events.Enqueue(ev);
            while (_events.Count > 5000 && _events.TryDequeue(out _)) { } // cap memory
        ...
        public object GetHealthPayload()
        {
            ...
            lock (_scheduleLock) schedule = _scheduleSnapshot.ToList();
    ...
    public class Monitoring : IHostedService
    {
        ...
        {
            if (!_options.Value.EnableHttpEndpoint) return Task.CompletedTask;
            ...
            try
            {
                ...
                _logger.LogInformation("Health HTTP endpoint listening on {Prefixes}", string.Join(", ", _options.Value.HttpPrefixes));
            }
            catch (Exception ex)
            {
        ...
        private void Handle(HttpListenerContext ctx)
        {
            try
            {
                ...
                {
                    if (!IsAuthorized(ctx)) { ctx.Response.StatusCode = 401; ctx.Response.Close(); return; }
                    var body = ReadBody(ctx);
                    var job = JsonSerializer.Deserialize<Dictionary<string, object>>(body);
                    CreateJob(ctx, job);
                }
                else if (path.StartsWith("/api/jobs/") && (ctx.Request.HttpMethod == "PUT" || ctx.Request.HttpMethod == "DELETE"))
                {
                    if (!IsAuthorized(ctx)) { ctx.Response.StatusCode = 401; ctx.Response.Close(); return; }
        ...
        private void ServeDashboard(HttpListenerContext ctx)
        {
            ...
            html = html.Replace("{{TITLE}}", _options.Value.Dashboard.Title)
                       ...
                       .Replace("{{RAW_TOGGLE}}", _options.Value.Dashboard.ShowRawJsonToggle
        ...
        private void ServeLogsTail(HttpListenerContext ctx)
        {
            ...
            if (Directory.Exists(logDir)) candidates.AddRange(SafeGetFiles(logDir, "*.txt"));
            if (Directory.Exists(logsDir)) candidates.AddRange(SafeGetFiles(logsDir, "*.txt"));
            ...
            var latest = candidates.Select(f => new FileInfo(f))
                                   .OrderByDescending(fi => fi.LastWriteTimeUtc)
                                   .First().FullName;

            int tailKb = 128;
            int.TryParse(ctx.Request.QueryString["tailKb"], out tailKb);
            if (tailKb <= 0 || tailKb > 4096) tailKb = 128;
        ...
        {
            if (job == null) { WriteJson(ctx, new { ok = false, error = "Invalid body" }, 400); return; }
            ...
            { WriteJson(ctx, new { ok = false, error = "Id, Command, CronExpression are required" }, 400); return; }

            try { CronExpression.Parse(Convert.ToString(job["CronExpression"])); }
            catch (Exception ex) { WriteJson(ctx, new { ok = false, error = $"Invalid cron: {ex.Message}" }, 400); return; }

            if (job.TryGetValue("TimeZone", out var tz) && !string.IsNullOrWhiteSpace(Convert.ToString(tz)))
            {
                try { _ = TimeZoneInfo.FindSystemTimeZoneById(Convert.ToString(tz)); }
                catch { WriteJson(ctx, new { ok = false, error = "Invalid TimeZone" }, 400); return; }
            }

            var list = ReadJobsRaw();
            var id = Convert.ToString(job["Id"]);
            if (list.Any(x => string.Equals(Convert.ToString(x.GetValueOrDefault("Id")), id, StringComparison.OrdinalIgnoreCase)))
            { WriteJson(ctx, new { ok = false, error = "Id already exists" }, 409); return; }
        ...
        {
            var list = ReadJobsRaw();
            var idx = list.FindIndex(x => string.Equals(Convert.ToString(x.GetValueOrDefault("Id")), id, StringComparison.OrdinalIgnoreCase));
            if (idx < 0) { WriteJson(ctx, new { ok = false, error = "Not found" }, 404); return; }

            incoming ??= new();
            incoming["Id"] = id;

            if (incoming.TryGetValue("CronExpression", out var cron) && !string.IsNullOrWhiteSpace(Convert.ToString(cron)))
            {
                try { CronExpression.Parse(Convert.ToString(cron)); }
                catch (Exception ex) { WriteJson(ctx, new { ok = false, error = $"Invalid cron: {ex.Message}" }, 400); return; }
            }
        ...
        {
            var list = ReadJobsRaw();
            var newList = list.Where(x => !string.Equals(Convert.ToString(x.GetValueOrDefault("Id")), id, StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        {
            if (string.IsNullOrWhiteSpace(configured)) return Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "dashboard.html");
        ...
        private void SetupHtmlWatcher(string path)
        {
            try
            {
                ...
                _htmlWatcher.Changed += (s, e) => { /* file changes are picked up on next request */ };
    ...
    internal static class StringReplaceExtensions
    {
        public static string replaceInsensitive(this string s, string find, string replaceWith)
            => s?.Replace(find, replaceWith, StringComparison.OrdinalIgnoreCase) ?? s;
    }

    public static class DictionaryExtensions
    {
        public static TValue GetValueOrDefault<TKey, TValue>(this IDictionary<TKey, TValue> d, TKey key, TValue fallback = default)
